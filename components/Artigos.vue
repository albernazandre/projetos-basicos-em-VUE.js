...
export default {
  ...
  methods: {
    fetchArtigos( url ){
      let virtual = this
      url = url || 'http://localhost/artigos/public/api/artigos'
      fetch( url )
        .then( res => res.json())
        .then( res => {
    ...
    cadastrarArtigos(){
      ...
      if( this.editar === false ){
        ...
        fetch('http://localhost/artigos/public/api/artigo', {
            method: 'post',
        ...
        fetch(`http://localhost/artigos/public/api/artigo/${this.artigo.id}`, {
            method: 'put',
            body: JSON.stringify( this.artigo ),
            headers: {
              'content-type': 'application/json'
    ...
    deletarArtigo( id ){
      if( confirm('Deseja deletar o artigo de id: ' + id) ){
        fetch(`http://localhost/artigos/public/api/artigo/${id}`, {
          method: 'delete'
        })
        .then( res => res.json())
        .catch( err => console.log( err ))
